...
namespace Render
{
	...
	typedef struct BlendStateKey
	{
		...
		{
			return std::hash<bool>()(rtDesc.BlendEnable) ^ std::hash<uint8_t>()(rtDesc.SrcBlend) ^ std::hash<uint8_t>()(rtDesc.DestBlend) ^ std::hash<uint8_t>()(rtDesc.SrcBlendAlpha) ^ std::hash<uint8_t>()(rtDesc.DestBlendAlpha) ^ std::hash<uint8_t>()(rtDesc.BlendOp) ^ std::hash<uint8_t>()(rtDesc.BlendOpAlpha) ^ std::hash<uint8_t>()(rtDesc.RenderTargetWriteMask);
		}
	...
	typedef struct RasterStateKey
	{
		...
		RasterStateKey(D3D11_RASTERIZER_DESC desc) :
			desc(desc) {}
		...
		{
			return desc.FillMode == other.desc.FillMode &&
